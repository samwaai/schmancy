/**
 * Standard number of decimal places for all quantities in the system
 * Set to 5 decimals to support:
 * - Meat/weight suppliers (3 decimals)
 * - High-precision clients (5 decimals)
 * - Future flexibility
 *
 * Change this constant to adjust precision system-wide
 */
export const QUANTITY_DECIMALS = 5

/**
 * Rounds a quantity to the standard decimal places
 * Use this for all quantity rounding in the system
 *
 * @param value - The quantity to round
 * @returns Rounded quantity with QUANTITY_DECIMALS precision
 *
 * @example
 * roundQuantity(1.234567) // returns 1.23457
 * roundQuantity(-0.0029699196) // returns -0.00297
 */
export function roundQuantity(value: number): number {
  const scale = Math.pow(10, QUANTITY_DECIMALS)
...
export class NumberFormatter {
  ...
  roundNumber(number: number, decimalPlaces: number = 2): number {
    // FIX: String concatenation creates invalid scientific notation (e.g. "1e-16e2")
