...
const todosStore = createContext<Todo>(
  new Map<string, Todo>(),
  'local',
  'todos'
);
...
Selectors let you create derived state that updates efficiently:

```typescript
import { createSelector, createCollectionSelector } from '@mhmo91/schmancy';

// Basic selector for an object store
...
const completedTodosSelector = createFilterSelector(
  todosStore,
  todo => todo.completed
);

completedTodosSelector.subscribe(completedTodos => {
  console.log('Completed todos:', completedTodos);
});

// Compound selectors that depend on multiple sources
const dashboardSelector = createCompoundSelector(
  [userSelector, todosSelector, settingsSelector],
  (user, todos, settings) => ({
    username: user.name,
...
// Array-specific selectors
const recentBooksSelector = createSelector(
  readingListStore,
  books => books.filter(book => book.yearPublished > 2000)
);
```
...
const gpt4Outputs = aiOutputsStore.filter(output => 
...
const conversationsStore = createContext<Conversation>(
...
const aiCacheStore = createContext<CachedResponse>(
...
const trainingStore = createContext<ModelTraining>(
  new Map<string, ModelTraining>(),
  'indexeddb',
  'model-training-jobs'
);
...
// Visualization selector for a specific training job
const trainingMetricsSelector = createSelector(
  trainingStore, 
  state => {
    const training = state.get(currentTrainingId);
    if (!training) return { epochs: [], loss: [], accuracy: [] };
    
    return {
      epochs: training.metrics.map(m => m.epoch),
      loss: training.metrics.map(m => m.loss),
      accuracy: training.metrics.map(m => m.accuracy)
    };
  }
);
```
...
const optimizedSelector = createOptimizedSelector(
  userStore,
...
const testTodosStore = createTestArrayContext<Todo>(
  [
