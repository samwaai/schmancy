      ...
      <div class="max-w-6xl mx-auto p-6" ${animate()}>
        ...
        <div class="mb-12 text-center">
          ...
          <schmancy-typography type="body" token="lg" class="text-on-surface-variant max-w-3xl mx-auto">
            Vertical navigation for desktop applications. Rails provide access to primary destinations while maintaining a compact footprint.
          </schmancy-typography>
        </div>
          ...
          <schmancy-surface type="container" class="p-6 rounded-xl">
            <div class="flex h-96 rounded-lg overflow-hidden bg-surface">
              <div class="w-20 bg-surface-container flex flex-col items-center py-4 gap-3">
                ${this.railItems.map(item => html`
                  <div
                    class="w-14 h-14 rounded-2xl flex items-center justify-center cursor-pointer transition-all
                           ${this.activeRailItem === item.id ? 'bg-secondary-container text-on-secondary-container' : 'text-on-surface-variant hover:bg-surface-container-high'}"
              ...
              <div class="flex-1 p-8 flex items-center justify-center text-on-surface">
                <div class="text-center">
                  <schmancy-icon size="xl" class="mb-4">${this.railItems.find(i => i.id === this.activeRailItem)?.icon}</schmancy-icon>
                  ...
                  <schmancy-typography type="body" token="md" class="mt-2 text-on-surface-variant">
                    Content for the ${this.railItems.find(i => i.id === this.activeRailItem)?.label.toLowerCase()} section would be displayed here.
                  </schmancy-typography>
                </div>
              </div>
            </div>
        ...
        <div class="mb-12">
          ...
          <schmancy-surface type="container" class="p-6 rounded-xl">
            <div class="mb-4">
              <schmancy-button
                variant="outlined"
                @click=${() => this.extendedRail = !this.extendedRail}
              >
                ${this.extendedRail ? 'Collapse' : 'Extend'} Rail
              </schmancy-button>
            </div>
            <div class="flex h-96 rounded-lg overflow-hidden bg-surface">
              <div class="${this.extendedRail ? 'w-48' : 'w-20'} bg-surface-container flex flex-col py-4 transition-all duration-300">
                ${this.railItems.map(item => html`
                  <div
                    class="flex items-center cursor-pointer transition-all mx-2 mb-2
                           ${this.extendedRail ? 'px-4 py-3 rounded-full justify-start gap-3' : 'w-14 h-14 rounded-2xl justify-center'}
                           ${this.activeRailItem === item.id ? 'bg-secondary-container text-on-secondary-container' : 'text-on-surface-variant hover:bg-surface-container-high'}"
                    @click=${() => this.activeRailItem = item.id}
                  >
                    <schmancy-icon>${item.icon}</schmancy-icon>
                    ${this.extendedRail ? html`
        ...
        <div class="mb-12">
          ...
          <schmancy-surface type="container" class="p-6 rounded-xl">
            <div class="flex h-96 rounded-lg overflow-hidden bg-surface">
              <div class="w-20 bg-surface-container flex flex-col items-center py-4 gap-3">
                ${this.badgeItems.map(item => html`
                  <div
                    class="w-14 h-14 rounded-2xl flex items-center justify-center cursor-pointer transition-all relative
                           ${this.railWithBadges === item.id ? 'bg-secondary-container text-on-secondary-container' : 'text-on-surface-variant hover:bg-surface-container-high'}"
                    @click=${() => this.railWithBadges = item.id}
                    title=${item.label}
                  >
                    <schmancy-icon>${item.icon}</schmancy-icon>
                    ${item.badge ? html`
                      <div class="absolute -top-1 -right-1 w-5 h-5 bg-error text-on-error rounded-full flex items-center justify-center text-xs font-medium">
                        ${item.badge}
                      </div>
                    ` : ''}
                  </div>
              ...
              <div class="flex-1 p-8 flex items-center justify-center text-on-surface">
                <div class="text-center">
                  <schmancy-icon size="xl" class="mb-4">${this.badgeItems.find(i => i.id === this.railWithBadges)?.icon}</schmancy-icon>
                  <schmancy-typography type="title" token="lg">
                    ${this.badgeItems.find(i => i.id === this.railWithBadges)?.label}
                  </schmancy-typography>
                  ${this.badgeItems.find(i => i.id === this.railWithBadges)?.badge ? html`
                    <schmancy-typography type="body" token="md" class="mt-2 text-on-surface-variant">
