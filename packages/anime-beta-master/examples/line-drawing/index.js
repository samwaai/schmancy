...
function generateLines(numberOfLines) {
  ...
  for (let i = 0; i < numberOfLines; i++) {
    ...
    svgContent += `<line x1="${x}" y1="${margin}" x2="${x}" y2="${svgHeight - margin}" class="line-v" stroke="#A4FF4F" stroke-linecap="butt" stroke-width="3"></line>`;
  }
...
function generateCircles(numberOfCircles) {
  ...
  for (let i = 0; i < numberOfCircles; i++) {
    ...
    svgContent += `<circle class="circle" stroke="#A4FF4F" stroke-linecap="butt" stroke-width="2" cx="${centerX}" cy="${centerY}" r="${radius}"></circle>`;
  }
...
const svgLines = generateLines(750);
const svgCircles = generateCircles(750);
