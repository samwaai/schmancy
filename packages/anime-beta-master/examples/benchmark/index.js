...
const renderModes = {
  'css-transform': cssTransformMode,
  'wappi-css-transform': WAAPICssTransformMode,
...
cssTransformMode.init = () => {

  if (cssTransformMode.isInitialized) return;

  cssTransformMode.isInitialized = true;
  ...
  cssTransformMode.cancel = () => {
    const $particles = document.querySelectorAll('.particle');
    utils.remove($particles);
    $particlesContainer.innerHTML = '';
  }

  cssTransformMode.refresh = () => {
    maxScale = utils.clamp(cssTransformMode.maxCount / config.count, .125, 5);
...
WAAPICssTransformMode.init = () => {

  if (WAAPICssTransformMode.isInitialized) return;

  WAAPICssTransformMode.isInitialized = true;
  ...
  WAAPICssTransformMode.cancel = () => {
    const animations = document.getAnimations();
    animations.forEach(animation => { animation.cancel() });
    $particlesContainer.innerHTML = '';
  }

  WAAPICssTransformMode.refresh = () => {
    maxScale = utils.clamp(WAAPICssTransformMode.maxCount / config.count, .125, 5);
    for (let i = 0; i < config.count; i++) {
      const $el = createParticule();
      $particlesContainer.appendChild($el);
      animateParticle($el, i, config.count);
    }
  }

  WAAPICssTransformMode.resize = () => {
    W = containerW;
    H = containerH;
    halfW = W * .5;
    halfH = H * .5;
