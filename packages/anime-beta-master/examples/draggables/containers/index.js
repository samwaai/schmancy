...
const [ $drawerContent ] = /** @type {Array<HTMLElement>} */(utils.$('.drawer-content'));
const [ $drawerTrigger ] = /** @type {Array<HTMLElement>} */(utils.$('.drawer-trigger'));
const [ $updateDrawerContent ] = /** @type {Array<HTMLElement>} */(utils.$('#update-drawer-content'));


const drawerAnim = createTimeline({
  autoplay: false,
...
const drawer = createDraggable($drawer, {
  ...
  onUpdate: (self) => {
    drawerAnim.progress = self.progressY;
...
$toggleDrawer.onclick = () => {
  drawer.stop(); // This kills all ongoing draggable related animations
  animate(drawer, {
...
$updateDrawerContent.onclick = () => {
  $drawer.style.removeProperty('height');
  const h1 = $drawer.offsetHeight;
  $drawerContent.appendChild($drawer.querySelector('p').cloneNode(true));
  $drawerContent.appendChild($drawer.querySelector('p').cloneNode(true));
  const h2 = $drawer.offsetHeight;
  animate($drawer, { height: [h1, h2], duration: 330, ease: 'out(3)' });
...
createDraggable('#nested-draggable', {
  container: '.drawer',
  containerPadding: [16, 16, 1016, 16],
});
