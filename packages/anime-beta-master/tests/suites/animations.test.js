...
suite('Animations', () => {
  ...
  test('Get Attribute tween type with input values', () => {
  ...
  test('Animation\'s tweens timing inheritance', () => {
    ...
    expect(animation._iterationDuration).to.equal(10 + 35 + 30 + 15 + 10);
  });
  ...
  test('Animate the progress of an animation with 0 duration tweens', resolve => {
    ...
    animate(anim1, {
      ...
      onComplete: self => {
        ...
        expect(self.currentTime).to.equal(self.duration);
        expect(anim1.progress).to.equal(1);
        expect(anim1.currentTime).to.equal(anim1.duration);
        resolve();
      }
    });
  });
  ...
  test('Animations should have advanced by one frame imediatly after beeing played', resolve => {
    ...
    createTimer({
      duration: 1,
      onComplete: () => {
        expect(anim1.currentTime).to.be.above(16);
