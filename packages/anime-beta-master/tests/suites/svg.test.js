...
suite('SVG', () => {
  test('svg.createDrawable', resolve => {
    ...
    animate(svg.createDrawable(['line', 'circle', 'polygon', 'polyline', 'rect', 'path']), {
      ...
      duration: 10,
      onComplete: () => {
        expect(line1El.getAttribute('stroke-dasharray')).to.equal('100000 0');
        expect(line2El.getAttribute('stroke-dasharray')).to.equal('100000 0');
        expect(circleEl.getAttribute('stroke-dasharray')).to.equal('100000 0');
        expect(polygonEl.getAttribute('stroke-dasharray')).to.equal('100000 0');
        expect(polylineEl.getAttribute('stroke-dasharray')).to.equal('100000 0');
        expect(rectEl.getAttribute('stroke-dasharray')).to.equal('100000 0');
        resolve();
      }
    });
  });
