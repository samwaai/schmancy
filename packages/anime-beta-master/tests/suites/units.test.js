...
suite('Units', () => {
  ...
  test('Basic unit conversion', () => {
    ...
    expect(el.offsetWidth).to.equal(20); // 1em = 20px
    utils.set(el, { width: 2 }); // Should inherit the 'em' unit
    expect(el.offsetWidth).to.equal(40); // 2em = 40px
    utils.set(el, { width: '100%' });
    expect(el.offsetWidth).to.equal(el.parentNode.offsetWidth - 2); // -2 = (1px border * 2)
    utils.set(el, { width: 50 }); // Should inherit the 'em' unit
    expect(el.offsetWidth).to.equal(Math.round((el.parentNode.offsetWidth - 2) / 2)); // 50% of parent 100% -2
    utils.set(el, { width: '50px' }); // Should inherit the 'em' unit
    expect(el.offsetWidth).to.equal(50);
    utils.set(el, { width: 'calc(100% - 2px)' }); // Calc should properly overiide from values
    expect(el.offsetWidth).to.equal(el.parentNode.offsetWidth - 4); // -4 = (1px border * 2) - 2
  });
