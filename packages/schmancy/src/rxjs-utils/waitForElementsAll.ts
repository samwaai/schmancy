 ...
 * @returns
 */
export default function waitForElementsAll<T extends Array<T>>(
  /**
  ...
  return observeOnMutation(document.body).pipe(
    ...
    take(1),
    map(() =>
      selectors.map(s => document.querySelectorAll(s)).filter(isPresent),
    ), // why filter again? see https://github.com/Microsoft/TypeScript/issues/16069
    map(elements => {
